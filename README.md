...
| `GONIC_COVERS_PATH`                 | `-covers-path`                 | **optional** path to a directory where gonic stores cover art set from a client. created for you if missing, and defaults to a `covers` directory next to the database - under docker that is `/data/covers`, on the volume you already mount. **must not be inside `GONIC_CACHE_PATH`**, which is periodically pruned, and is never the music directory: gonic only ever reads your music tree. wherever you point it, make sure it is persistent. |
