...
| `GONIC_COVERS_PATH`                 | `-covers-path`                 | path to a directory where gonic stores cover art set from a client. defaults to `covers` next to the database. **must not be inside `GONIC_CACHE_PATH`**, which is periodically pruned, and is never the music directory: gonic only ever reads your music tree. give this its own persistent volume. |
